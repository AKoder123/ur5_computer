...
def callback(position):
    ...
    print(round(position.linear.z, 3))
    
    wpose.position.x = wpose.position.x
    wpose.position.y = wpose.position.y
    wpose.position.z = round(position.linear.z, 3)
...
if __name__ == "__main__":
    ...
    control_subscriber = rospy.Subscriber("/ur5_move_end_effector", Twist, callback, queue_size=1)
