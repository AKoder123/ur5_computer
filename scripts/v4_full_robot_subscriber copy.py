...
def callback(direction):
    ...
    if True:
        ...
        delta_roll += direction.angular.x * scale
        delta_pitch += direction.angular.y * scale
        delta_yaw += direction.angular.z * scale
        ...
        #position
        current_position = np.array([wpose.position.x, wpose.position.y, wpose.position.z])
        current_orientation = wpose.orientation
        current_quat = [current_orientation.x, current_orientation.y, current_orientation.z, current_orientation.w]
        ...
        # Transform translation vector to local frame
        global_translation = np.array([direction.linear.x, direction.linear.y, direction.linear.z]) 
        local_translation = np.dot(rotation_matrix, global_translation)

        # Update position in global frame
        new_position = current_position + local_translation
        wpose.position.x = new_position[0]
        wpose.position.y = new_position[1]
